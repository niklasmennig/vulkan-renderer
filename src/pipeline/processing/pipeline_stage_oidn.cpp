...
void ProcessingPipelineStageOIDN::on_resize(VkExtent2D swapchain_extent, VkExtent2D render_extent) {
    ...
    HANDLE input_handle, output_handle, albedo_handle, normals_handle;
    ...
    oidn_buffer_output = oidnNewSharedBufferFromWin32Handle(oidn_device, OIDN_EXTERNAL_MEMORY_TYPE_FLAG_OPAQUE_WIN32, output_handle, nullptr, output_buffer->buffer_size);
    ...
    oidnSetFilterImage(oidn_filter, "output", oidn_buffer_output, OIDN_FORMAT_FLOAT3, render_extent.width, render_extent.height, 0, sizeof(float) * 4, 0);
